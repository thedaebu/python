...
# Phase 2
  ...
  def contains_value(self, target):
    ...
    while current is not None:
      if current._value is target: return True
      current = current._next
    return False
  ...
  def insert_value(self, position, value):
    ...
    if position is 0: return self.add_to_head(value)
    if position is self._length-1: return self.add_to_tail(value)
  ...
  def remove_node(self, position):
    if position < 0 or position >= self._length: return False
    if position is 0: return self.remove_head()
    if position is self._length-1: return self.remove_tail()
    removed = current_node._next
    current_node = self.get_node(position)
  ...
  def __str__(self):
    if self._length is 0: return "Empty List"
    current = self._head
    final = f"{str(current._value)}"
    current = current._next
    while current is not None:
      final = f"{final}, {str(current._value)}"
      current = current._next
    return final
